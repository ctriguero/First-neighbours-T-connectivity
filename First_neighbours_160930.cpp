...
AtomSet::AtomSet(size_t dim_AtomSetCard) : AtomSetCard(dim_AtomSetCard), AtomList()
{
	...
	std::ifstream file( "test.xyz" ) ; // input file
...
int main()
{
	...
	size_t Natoms = 22 ;
	//unsigned int Nnodes = 333 ;
	//double RT = 0.5 ; // Angstroms
	//double RT = 5.0 ;
	double RT = 1.0 ;
	//double RT = 3.4 ;
	...
	for ( size_t i = 0; i < Natoms; ++i ) {
		if ( AtomSet_object.GetAtom(i).GetType() == 1 ) {
		ifriends = 0 ;
		for ( size_t j = 0; j < Natoms; ++j ) {
			if ( AtomSet_object.GetAtom(j).GetType() == 1 ) { 
